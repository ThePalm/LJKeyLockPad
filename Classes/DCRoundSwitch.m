...
#import "DCRoundSwitch.h"               // include of this class
#import "DCRoundSwitchToggleLayer.h"    // the sliding layer
#import "DCRoundSwitchOutlineLayer.h"   // outlinelayer that hides the slider sticking out past the outline of the control
#import "DCRoundSwitchKnobLayer.h"      // round knob drawn on the sliding layer
...
- (void)setup
{
	...
	// this layer contains the onTintColor (blue by default), the text, and the shadow for the knob.  the knob shadow is
	...
	UITapGestureRecognizer *tapGestureRecognizer = [[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tapped:)] autorelease]; //call tapped when tapped
	[self addGestureRecognizer:tapGestureRecognizer];    // add to self apparently UIControl Base class has the add gesture recognizer

	// pan gesture for moving the switch knob manually
	UIPanGestureRecognizer *panGesture = [[[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(toggleDragged:)] autorelease];
    [self addGestureRecognizer:panGesture];              // add to self apparently UIControl Base class has the add gesture recognizer       
...
- (void)tapped:(UITapGestureRecognizer *)gesture    //tapped gesture recognizer fired this event baased on action parm when constructed.
{
	if (ignoreTap) return;
...
- (void)toggleDragged:(UIPanGestureRecognizer *)gesture     //draggeded gesture recognizer fired this event baased on action parm when constructed.
{
